// Parley proxy — agent-settings extension.
//
// Two routes:
//
...
export async function handleParleySettingsSchema(_req, res) {
  ...
  try {
    ...
    console.warn('[parley] settings schema fetch failed:', e?.message);
  ...
  // filter (e.g. hermes-plugin reads its `parley.preferred_models`
  // config and pre-filters before returning). Parley is a thin
  // forward.
...
export async function handleParleySettingsUpdate(req, res, id: string) {
  const upstream = getUpstream();
  if (!upstream) {
    res.writeHead(503, { 'content-type': 'application/json' });
    res.end(JSON.stringify({ error: 'parley_platform_unconfigured' }));
  ...
  try {
    ...
    console.warn(`[parley] settings update failed for ${id}:`, e?.message);
    res.writeHead(502, { 'content-type': 'application/json' });
