...
export async function handleParleyMessage(req, res) {
  ...
  if (aborted) {
    console.warn(
      `[parley] /api/parley/messages aborted: body > ${MAX_BODY_BYTES} bytes ` +
  ...
  // Forward attachments via the parley extension to /v1/responses
  ...
  // gets dropped.
  console.log(
    `[parley:messages] POST chat=${chatId} ` +
...
async function dispatchTurnViaUpstream(
  ...
  const t0 = Date.now();
  let envelopeCount = 0;
  console.log(`[parley:dispatch] start chat=${chatId}`);
  try {
    for await (const envelope of upstream.sendMessage(chatId, text, {
      ...
      console.log(
        `[parley:dispatch] +${Date.now() - t0}ms envelope #${envelopeCount} ` +
    ...
    console.log(
      `[parley:dispatch] end chat=${chatId} ` +
      `envelopeCount=${envelopeCount} elapsed=${Date.now() - t0}ms`,
    );
  } catch (e: any) {
    console.error(
      `[parley:dispatch] FAILED chat=${chatId} ` +
      `envelopeCount=${envelopeCount} elapsed=${Date.now() - t0}ms ` +
