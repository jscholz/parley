...
beforeEach(async () => {
  dir = await fs.mkdtemp(path.join(os.tmpdir(), 'sidekick-media-test-'));
...
test('rejects paths outside the allowed roots', async () => {
  const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'sidekick-media-outside-'));
  try {
...
test('rejects a symlink that escapes the allowed roots', async () => {
  const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'sidekick-media-target-'));
  try {
