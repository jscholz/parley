...
// for parley-owned chats, so non-parley activity doesn't show up
...
//
// Test plan (mocked):
//   1. Pre-populate parley chat A. Drawer renders A.
...
export const DESCRIPTION = 'Drawer auto-refreshes (visibility change) to surface non-parley chats';
...
export default async function run({ page, log, mock }) {
  ...
  log(`drawer pre-populated with parley chat only ✓`);
