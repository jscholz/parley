...
// the drawer alongside parley chats, render with a source badge,
...
//
// Phase 1 of the cross-platform-visibility design: parley sessions
// mirror state.db / sessions transcripts on disk.
//
// Test plan (mocked):
//   1. Pre-populate 1 parley chat (with content) + 1 telegram chat.
//   2. Click parley chat → assert composer ENABLED + no badge on
//      this row's neighbors that we care about (parley is the
...
//   4. Click parley chat → composer ENABLED again.
...
export default async function run({ page, log }) {
  ...
  log('drawer pre-populated: 1 parley + 1 telegram chat ✓');

  // 1. Source badge on telegram row but NOT parley row.
  ...
  const badgeInfo = await page.evaluate(({ sk, tg }) => {
    ...
    return { parley: scan(sk), telegram: scan(tg) };
  ...
  assert(
    Array.isArray(badgeInfo.parley) && badgeInfo.parley.length === 0,
    `parley row should have no source badge, got ${JSON.stringify(badgeInfo.parley)}`,
  );
  log(`telegram badge present; parley row has no badge ✓`);

  // 2. Click parley — composer enabled.
  ...
  assert(state.inputDisabled === false, `step 2: composer should be enabled on parley chat, got disabled=${state.inputDisabled}`);
  assert(!state.hasReadonlyClass, `step 2: composer should NOT have .readonly class on parley`);
  log(`parley chat: composer enabled ✓`);
  ...
  assert(
    ...
    `step 3: parley marker should not appear in telegram chat`,
  );
  log(`telegram chat: composer disabled with read-only placeholder ✓`);

  // 4. Click back to parley — composer re-enabled.
  await clickRow(page, SK_CHAT);
  await page.waitForFunction(
    (m) => (document.getElementById('transcript')?.textContent || '').includes(m),
    SK_MARKER,
  ...
  await page.waitForTimeout(100);
  state = await composerState(page);
  assert(state.inputDisabled === false, `step 4: composer should be enabled after returning to parley, got disabled=${state.inputDisabled}`);
  assert(!state.hasReadonlyClass, `step 4: composer should NOT have .readonly class`);
  log(`returned to parley: composer re-enabled ✓`);
}
