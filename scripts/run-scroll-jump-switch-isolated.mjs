#!/usr/bin/env node
/**
 * Self-contained runner for scripts/scroll-jump-switch-repro.mjs.
 *
...
try {
  ...
  console.error(`[scroll-switch-diag:isolated] ${e.message}`);
  console.error(serverLog.join('').split('\n').slice(-20).join('\n'));
  teardown();
  process.exit(2);
}

console.log(`[scroll-switch-diag:isolated] server :${port}  home ${home}  worktree ${ROOT}`);

const harness = spawn(
  process.execPath,
  [path.join(__dirname, 'scroll-jump-switch-repro.mjs'), ...process.argv.slice(2), `--base-url=http://127.0.0.1:${port}`],
